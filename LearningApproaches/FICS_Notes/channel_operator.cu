...
/**
* 本文件的内容，大体可以分成两类，一类是CUDA Kernel函数，一类是完成Kernel函数配置的host函数接口，
* 他们两个具有类似的函数名字，除了CUDA Kernle函数的函数名字末尾有Kernel字样，并以__global__开头。
*/

#define ChannelOperator_CUDA_CHECK(condition) \
  /* Code block avoids redefinition of cudaError_t error */ \
  /*
  * 使用do...while(0)是为了独立的命名空间；另一方面，是为了在进行宏替换的时候出现意想不到的情况。
  */
  do { \
...
/**
*  用于Kernel function里面的单个线程里面的循环。
*  从代码上来看，只适合一维Thread Block和每一个Block是一维Thread的情况
*/
...
namespace mshadow {
  namespace cuda {
    /**
    *   定义Kernel函数，被GroupMaxForward调用！
    *     GroupMaxForward被channel_operator_inl.h调用。
    *   注意学习这里的模板在Kernel函数中的应用！
    */
    ...
    */
    ...
    inline void GroupMaxForward(const Tensor<gpu, 4, DType> &out,
      ...
      /**
      *   注意这里调用CUDA Kernel的方式，尤其是Kernel function configure的方式
      *   使用cuda_get_num_blocs得到Grid里面的Block的维度
      *   使用kBaseThreadNum作为每一个Block里面的Thread的维度
      */
      ...
      /**
      *   调用cudaPeekAtLastError()来获取可能存在的最近的一次cuda的runtime错误
      */
    ...
    /**
    * 用于channel_operator-inl.h 文件里面Backward函数里面调用的函数
    */  
